...
| Rules (`rules/`) | `firebase emulators:exec --only firestore "python3 tests/rules/emulator_rules_test.py"` | That the deployed rules text actually enforces crew scoping, consent, symmetric board reads, opt-out retraction, and the retired collection. | Live Anki behavior. |
